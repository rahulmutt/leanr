...
- Expected type propagated to the body (not the value) → Task 2 Step 4 (`elab_term_ensuring_type(&body_elem, kinds, expected)`); oracle-faithful by construction (mirrors `elabTermEnsuringType body expectedType?`), but **unobservable until M4b-3 brings applications/postponement** — no applications, no coercions, no postponement, and the outer `elab_term_ensuring_type` re-checks the whole `letE` anyway. Corpus `let/ascribed`/`have/ascribed` is byte-identical to `let/typed`/`have/typed` (:774) and carries zero discriminating power here; the whole-branch review mutation-tested this (flipping the body call's `expected` argument to `None` leaves `cargo test -p leanr_elab --test oracle_elab --test binder_smoke` fully green, 23/23). Not differentially verified — no ✔; tracked as a to-verify item under Known follow-ups.
...
**Known follow-ups (out of plan 3, named for later slices):** the entire `synthesizeSyntheticMVars` fixpoint + postponement + `TypeClass`/default-instance/error pass, application/`@`/named args, coercions, num/char literals, implicit and instance let binders (M4b-3); `letI`/`haveI`/`let_fun`/`let_delayed`/`let_tmp`/`letrec` and `let rec` lifting (later M4, each needing its own oracle tier); real macro-scope hygiene (whichever slice first needs it); **to-verify (M4b-3):** the `let`/`have` body's expected-type-propagation target (`elab_term_ensuring_type(&body_elem, kinds, expected)`) is oracle-faithful by construction but unobservable by any corpus fixture until M4b-3's applications/postponement give it a discriminating context (see Self-Review above).
