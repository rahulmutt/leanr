...
- **Parse errors carry stable codes** (spec §Error handling): `E0301` unexpected token/expected-one-of, `E0302` unterminated string, `E0303` unterminated block comment, `E0304` invalid escape sequence, `E0305` invalid UTF-8 (CLI boundary), `E0306` unterminated `«` identifier escape. Codes never change meaning once shipped.
