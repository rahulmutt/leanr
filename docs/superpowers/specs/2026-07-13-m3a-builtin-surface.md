...
4. **The toolchain's own comments contain example
   ...
   (a *separate*, additive bug from the two above) —
   found in `Lean/Parser/Term.lean` (a rejected `depArrow` design,
   inside a `/- ... -/` block), `Lean/Parser/Do.lean` (`doReassignElse`,
   behind a `--` line comment), `Lean/Attributes.lean` (a `/-! -/`
   module doc illustrating `Attr.lean`'s own declarations),
   `Lean/Meta/RecursorInfo.lean` (a `/- -/`-commented-out `recursor`),
   and `Lean/Elab/StructInst.lean` (a `structInstFieldWhere` example
   inside a doc comment — this name has **no live declaration anywhere**
   in the toolchain; it is aspirational text, not a parser). Plain grep
   counts all of these as hits; a real compiler wouldn't. Confirmed via
   a character-level scan tracking nested `/- -/` depth, `--` line
   comments, and `"..."` string literals (`\`-escaped) — the same class
   of bug a hand-rolled Lean tokenizer must get right anyway, so
   validating it here doubles as an early sanity check on that logic.
...
| parser | source | M3a status |
|---|---|---|
| simple | Attr.lean:39 | port |
| «macro» | Attr.lean:41 | port (the `@[macro foo]` attribute value, distinct from the `macro` command) |
| «export» | Attr.lean:42 | port |
| recursor | Attr.lean:45 | port |
| «class» | Attr.lean:46 | port |
| «instance» | Attr.lean:47 | port |
| default_instance | Attr.lean:48 | port |
| «specialize» | Attr.lean:49 | port |
| extern | Attr.lean:53 | port |
| «tactic_alt» | Attr.lean:63 | port |
| «tactic_tag» | Attr.lean:71 | port |
| «tactic_name» | Attr.lean:84 | port |
...
| parser | source | M3a status |
|---|---|---|
| moduleDoc | Command.lean:59 | port |
| declaration | Command.lean:282 | port (the `def`/`theorem`/`instance`/… dispatcher — fixture-critical) |
| «deriving» | Command.lean:286 | port (syntactic clause only; handler dispatch is elaboration, M4) |
| «section» | Command.lean:299 | port |
| «namespace» | Command.lean:317 | port |
| withWeakNamespace | Command.lean:330 | port |
| «end» | Command.lean:337 | port |
| «variable» | Command.lean:471 | port |
| «universe» | Command.lean:531 | port |
| check | Command.lean:533 | port |
| check_failure | Command.lean:535 | port |
| importPath | Command.lean:541 | port |
| assertNotExists | Command.lean:547 | port |
| assertNotImported | Command.lean:553 | port |
| checkAssertions | Command.lean:560 | port |
| eval | Command.lean:586 | port |
| evalBang | Command.lean:588 | port |
| synth | Command.lean:590 | port |
| exit | Command.lean:592 | port (`#exit` — trivial, still port) |
| print | Command.lean:594 | port |
| printSig | Command.lean:596 | port |
| printAxioms | Command.lean:600 | port |
| printEqns | Command.lean:602 | port |
| printTacTags | Command.lean:607 | port |
| «where» | Command.lean:614 | port |
| version | Command.lean:617 | port |
| withExporting | Command.lean:623 | port |
| dumpAsyncEnvState | Command.lean:626 | port |
| deprecatedSyntax | Command.lean:635 | port |
| «init_quot» | Command.lean:637 | port (bootstrap no-arg command; trivial) |
| «docs_to_verso» | Command.lean:642 | port |
| «deprecated_module» | Command.lean:656 | port |
| showDeprecatedModules | Command.lean:663 | port |
| «set_option» | Command.lean:682 | port (fixture-relevant: `set_option ... in <command>`) |
| «unlock_limits» | Command.lean:688 | port |
| «attribute» | Command.lean:692 | port |
| «export» | Command.lean:720 | port |
| «import» | Command.lean:722 | port |
| «open» | Command.lean:852 | port |
| «mutual» | Command.lean:855 | port |
| «initialize» | Command.lean:860 | port |
| «in» | Command.lean:864 | port (`... in <command>` wrapper) |
| addDocString | Command.lean:889 | port |
| «register_tactic_tag» | Command.lean:897 | port |
| «tactic_extension» | Command.lean:907 | port |
| «recommended_spelling» | Command.lean:948 | port |
| genInjectiveTheorems | Command.lean:958 | port |
| «include» | Command.lean:968 | port |
| «omit» | Command.lean:976 | port |
| registerErrorExplanationStx | Command.lean:1008 | port |
| «mixfix» | Syntax.lean:92 | **defer** — ParserDescr-producing notation DSL; parsing its own shape is trivial, but it exists only to *register* a parser extension, meaningless before M3b's ParserDescr interpreter. Lands M3b. |
| «notation» | Syntax.lean:95 | **defer** — same reason as «mixfix» |
| «macro_rules» | Syntax.lean:102 | **defer** — pattern-matches against quotations; needs antiquotation + macro-expansion machinery. M3b/M4. |
| «syntax» | Syntax.lean:105 | **defer** — ParserDescr-producing. M3b |
| syntaxAbbrev | Syntax.lean:108 | **defer** — ParserDescr-producing. M3b |
| syntaxCat | Syntax.lean:113 | **defer** — declares a new parser *category*; meaningless before the category/extension machinery lands. M3b |
| «macro» | Syntax.lean:119 | **defer** — ParserDescr + macro-expansion. M3b |
| «elab_rules» | Syntax.lean:122 | **defer** — same shape as macro_rules, targets elaborators. M3b/M4 |
| «elab» | Syntax.lean:127 | **defer** — ParserDescr-producing. M3b |
| binderPredicate | Syntax.lean:137 | **defer** — ParserDescr-producing (`binder_predicate` registers a new binder notation). M3b |
| grindPattern | Lean/Meta/Tactic/Grind/Parser.lean:140 | port (fixed shape: ident, `=>`, term list; grind-specific but trivial to parse) |
| initGrindNorm | Lean/Meta/Tactic/Grind/Parser.lean:143 | port |
...
| parser | source | M3a status |
|---|---|---|
| ident | Term.lean:124 | port |
| num | Term.lean:126 | port |
| scientific | Term.lean:128 | port |
| str | Term.lean:130 | port |
| char | Term.lean:132 | port |
| type | Term.lean:135 | port |
| sort | Term.lean:138 | port |
| prop | Term.lean:143 | port |
| «sorry» | Term.lean:164 | port |
| cdot | Term.lean:174 | port |
| typeAscription | Term.lean:182 | port |
| tuple | Term.lean:186 | port |
| paren | Term.lean:200 | port |
| anonymousCtor | Term.lean:216 | port |
| hole | Term/Basic.lean:118 | port |
| syntheticHole | Term/Basic.lean:164 | port |
| omission | Term/Basic.lean:175 | port |
...
| parser | source | M3a status |
|---|---|---|
| depArrow | Term.lean:256 | port |
| «forall» | Term.lean:259 | port |
| «match» | Term.lean:330 | port |
| «nomatch» | Term.lean:338 | port |
| «nofun» | Term.lean:340 | port |
| structInst | Term.lean:351 | port |
| structInstDefault | Term.lean:368 | port |
| «fun» | Term.lean:385 | port |
| «let» | Term.lean:550 | port |
| «have» | Term.lean:558 | port |
| «let_fun» | Term.lean:563 | port |
| «let_delayed» | Term.lean:568 | port |
| «let_tmp» | Term.lean:574 | port |
| «haveI» | Term.lean:577 | port |
| «letI» | Term.lean:580 | port |
| «letrec» | Term.lean:721 | port |
| «suffices» | Term.lean:225 | port |
| «show» | Term.lean:227 | port |
| explicit | Term.lean:232 | port |
| inaccessible | Term.lean:238 | port |
| byTactic | Term.lean:107 | port (fixture-critical: `by` blocks) |
| «unsafe» | Term.lean:761 | port |
| «open» | Command.lean:1018 | port (`open Foo in <term>` — term-scoped `open`, distinct from the command-category `«open»` above and the tactic-category `«open»` below) |
| «set_option» | Command.lean:1025 | port (`set_option ... in <term>` — term-scoped, distinct from the command/tactic versions) |

### App/projection/structural machinery — fixture-critical, all port

| parser | source | M3a status |
|---|---|---|
| app | Term.lean:892 | port (term application — the core Pratt-loop leading/trailing pair) |
| proj | Term.lean:906 | port |
| completion | Term.lean:908 | port |
| arrow | Term.lean:910 | port |
| dotIdent | Term.lean:924 | port |
| explicitUniv | Term.lean:943 | port |
| namedPattern | Term.lean:948 | port |
| pipeProj | Term.lean:957 | port |
| pipeCompletion | Term.lean:959 | port |
| subst | Term.lean:974 | port |
| panic | Term.lean:988 | port |
| unreachable | Term.lean:991 | port |
| dbgTrace | Term.lean:997 | port |
| borrowed | Term.lean:406 | port |
| quotedName | Term.lean:409 | port |
| doubleQuotedName | Term.lean:416 | port |
| noindex | Term.lean:747 | port |
| binrel | Term.lean:764 | port |
| binrel_no_prop | Term.lean:767 | port |
| binop | Term.lean:770 | port |
| binop_lazy | Term.lean:773 | port |
| leftact | Term.lean:777 | port |
| rightact | Term.lean:781 | port |
| unop | Term.lean:784 | port |
| forInMacro | Term.lean:787 | port |
| forInMacro' | Term.lean:789 | port |

### `do`-block term wrappers — fixture-critical, all port

| parser | source | M3a status |
|---|---|---|
| doForward | Do.lean:228 | port |
| nestedAction | Do.lean:24 | port |
| «do» | Do.lean:325 | port |
| termUnless | Do.lean:334 | port |
| termFor | Do.lean:336 | port |
| termTry | Do.lean:338 | port |
| termReturn | Do.lean:344 | port |
...
| parser | source | M3a status |
|---|---|---|
| stateRefT | Term.lean:1025 | port |
| showTermElabImpl | Term.lean:1034 | port |
| matchExpr | Term.lean:1056 | port |
| letExpr | Term.lean:1059 | port |
| throwNamedErrorMacro | Term.lean:1067 | port |
| throwNamedErrorAtMacro | Term.lean:1076 | port |
| logNamedErrorMacro | Term.lean:1084 | port |
| logNamedErrorAtMacro | Term.lean:1093 | port |
| logNamedWarningMacro | Term.lean:1101 | port |
| logNamedWarningAtMacro | Term.lean:1110 | port |
| declName | Term.lean:793 | port |
| «privateDecl» | Term.lean:796 | port |
| withDeclName | Term.lean:805 | port |
| typeOf | Term.lean:807 | port |
| ensureTypeOf | Term.lean:809 | port |
| ensureExpectedType | Term.lean:811 | port |
| noImplicitLambda | Term.lean:813 | port |
| «inferInstanceAs» | Term.lean:846 | port |
| valueOf | Term.lean:851 | port |
| clear | Term.lean:858 | port |
| letMVar | Term.lean:861 | port |
| waitIfTypeMVar | Term.lean:863 | port |
| waitIfTypeContainsMVar | Term.lean:865 | port |
| waitIfContainsMVar | Term.lean:867 | port |
| defaultOrOfNonempty | Term.lean:870 | port |
| noErrorIfUnused | Term.lean:877 | port |
| «idbg» | Term.lean:1001 | port |
| assert | Term.lean:1005 | port |
| debugAssert | Term.lean:1011 | port |
| elabToSyntax (`_root_.Lean.Parser.Term.elabToSyntax`) | Lean/Elab/Term/TermElabM.lean:815 | port (fixed `elabToSyntax% <numLit>` shape) |

### Parser-authoring meta-DSL — port (trivial wrapper syntax; some Mathlib meta files define custom `Parser`s directly this way)

| parser | source | M3a status |
|---|---|---|
| «leading_parser» | Term.lean:392 | port |
| «trailing_parser» | Term.lean:394 | port |

### Quotation family — **defer**, explicitly called out by the task brief as the paradigm case

| parser | source | M3a status |
|---|---|---|
| Term.quot | Command.lean:20 | **defer** — syntax quotation `` `(term) `` ; meaningless without antiquotation machinery, lands with M3b |
| Term.precheckedQuot | Command.lean:22 | **defer** — same reason |
| quot | Command.lean:50 | **defer** — command-category quotation, same reason |
| dynamicQuot | Term.lean:1028 | **defer** — same reason |
| Tactic.quot | Term.lean:1123 | **defer** — same reason |
| Tactic.quotSeq | Term.lean:1125 | **defer** — same reason |
...
| parser | source | M3a status |
|---|---|---|
| doLet | Do.lean:77 | port |
| doLetElse | Do.lean:79 | port |
| doLetExpr | Do.lean:83 | port |
| doLetMetaExpr | Do.lean:87 | port |
| doLetRec | Do.lean:91 | port |
| doLetArrow | Do.lean:99 | port |
| doReassign | Do.lean:112 | port |
| doReassignArrow | Do.lean:122 | port |
| doHave | Do.lean:124 | port |
| doIf | Do.lean:169 | port |
| doUnless | Do.lean:175 | port |
| doFor | Do.lean:186 | port |
| doMatch | Do.lean:193 | port |
| doMatchExpr | Do.lean:200 | port |
| doTry | Do.lean:209 | port |
| doBreak | Do.lean:232 | port |
| doContinue | Do.lean:234 | port |
| doReturn | Do.lean:245 | port |
| doDbgTrace | Do.lean:251 | port |
| doIdbg | Do.lean:281 | port |
| doAssert | Do.lean:286 | port |
| doDebugAssert | Do.lean:292 | port |
| doRepeat | Do.lean:295 | port |
| doWhile | Do.lean:297 | port |
| doRepeatUntil | Do.lean:299 | port |
| doExpr | Do.lean:318 | port |
| doNested | Do.lean:322 | port |
...
| parser | source | M3a status |
|---|---|---|
| paren | Level.lean:24 | port |
| max | Level.lean:26 | port |
| imax | Level.lean:28 | port |
| hole | Level.lean:30 | port |
| num | Level.lean:32 | port |
| ident | Level.lean:34 | port |
| addLit | Level.lean:36 | port |
...
| parser | source | M3a status |
|---|---|---|
| «unknown» | Tactic.lean:29 | port — fallback/error-recovery node for unrecognized tactic syntax; needed for the "parse errors are values" guarantee |
| nestedTactic | Tactic.lean:32 | port |
| «match» | Tactic.lean:50 | port |
| introMatch | Tactic.lean:55 | port |
| «open» | Command.lean:1032 | port (`open Foo in <tactic>`) |
| «set_option» | Command.lean:1037 | port (`set_option ... in <tactic>`) |
...
| parser | source | M3a status |
|---|---|---|
| paren | Syntax.lean:37 | port |
| cat | Syntax.lean:39 | port |
| unary | Syntax.lean:41 | port |
| binary | Syntax.lean:43 | port |
| sepBy | Syntax.lean:45 | port |
| sepBy1 | Syntax.lean:48 | port |
| atom | Syntax.lean:51 | port |
| nonReserved | Syntax.lean:54 | port |
| unicodeAtom | Syntax.lean:57 | port |

## struct-instance-field-decl category (`Lean/Parser/Term.lean`) — 2, both port

| parser | source | M3a status |
|---|---|---|
| structInstFieldDef | Term.lean:357 | port |
| structInstFieldEqns | Term.lean:360 | port |
...
| parser | kind | source | M3a status |
|---|---|---|---|
| numPrec | builtin_prec_parser | Syntax.lean:35 | port |
| numPrio | builtin_prio_parser | Attr.lean:33 | port |
