...
`1.5`), so the swap is free. P2b then lands with a live, non-degenerate
...
Six plans (P2 split per § Amendment; P3 before P2b per § Amendment 2),
each a single PR with its own hermetic oracle tier extending
`oracle_elab.rs` + `elab-queries.jsonl`. The table is in **execution
order**.
...
| **P3** literals + defaults | the `synthetic.rs` split and the fixture second-instances (§ Amendment 2 item 3), `Term.mkInstMVar`, `num` (`OfNat`), `char` (`Char.ofNat`), `scientific` (`OfScientific`), `synthesizeUsingDefault` / `synthesizeSomeUsingDefaultPrio` | `42`, `(42 : Tag)`, `'a'`, `1.5`, and a numeral inside an application |
...
the branch's own trigger needs a numeral (§ Amendment 2 item 1). P4
...
*Executes after § P3, per § Amendment 2. It keeps its position in this
document so the two halves of the P2 split read together.*
...
which is why P2b follows both.

Two things about the corpus follow from that, and are what make P2b
worth running only here. The env gate
(`env.contains ``Lean.Internal.coeM`, `App.lean:1355`) is false
throughout `Elab0.lean` today, so P2b's fixture task declares
...
but as a *temporary* state across the two PR boundaries it now spans
(P2a, then P3) it is sound, because P1 already shipped it as a named,
shape-guarded seam rather than a silent divergence (§ Amendment, item 1).
...
`'a'`, `1.5`, plus a numeral *inside* an application and one against an
instance goal — the application cases are what actually drive the
ladder, since a bare `42` with no expected type is the degenerate path.
...
P2b, § Amendment 2). P3's row is the one entry that is not purely
additive; it says so.
...
| P3 | Audited against the merged P2a code; larger than this spec's original three. **Additive forwarders:** `get_dec_level` (composes the private `get_level`, `infer.rs:751`, `normalize_level`, and `dec_level`, `level.rs:448`); `is_prop` (widening the `pub(crate)` `lazy_delta.rs:161` to `pub` — needed by `num`'s Prop failure branch; no name clash exists, so no forwarder was added); `checkpoint`/restore (forwarding the `pub(crate)` `metactx.rs:937`, needed for `commitWhen`); `with_assignable_synthetic_opaque` (used only by `synthesizeUsingDefaultInstance`, `SyntheticMVars.lean:164` — the spec's earlier attribution to `synthesizeUsingDefaultPrio` was off by one function; corrected against the pinned v4.33.0-rc1 source in M4b-3 P3 task 4 — moved here from P2a, whose ladder never reaches it; `config.rs:27` already names `assign_synthetic_opaque` as arriving). `mk_raw_nat_lit` is NOT needed (M4b-3 P3 task 4): `Store::expr_lit_nat` (`leanr_kernel/src/bank/terms.rs:535`) and `MetaCtx::store_mut` are both already public, which is exactly how `builtin/lit.rs`'s `elab_str` reaches `expr_lit_str`. **New:** `default_instance_priorities` — the *global* descending distinct priority set (`getDefaultInstancesPriorities`); the existing `default_instances_of` is per-class and cannot produce it. **Not additive, and the one item that isn't:** `mk_const_with_fresh_mvar_levels` and `forall_meta_telescope_reducing` (returning binder infos, which `synthesizeUsingDefaultInstance` needs to pick out the `instImplicit` binders as new pending goals). Both loops exist — `refresh_instance_levels`, `synth.rs:2006`, and `get_subgoals`, `synth.rs:1936` — but private, specialized to `&Instance`, and discarding binder infos. P3 **generalizes them out of `synth.rs` and has `get_subgoals` call the generalized form**, rather than duplicating a fidelity-critical telescope loop in `leanr_elab`. Behavior-neutral, gated by `synth.rs`'s existing tests plus the `leanr_meta` oracle corpus staying byte-identical. |
...
#32. The next implementation plan is **P3** — literals and default
instances (§ P3, and § Amendment 2 for why it now precedes P2b and what
it absorbs from P2a's deferrals). P2b, P4 and P5 get their own
