...
//!
//! - [`MetaCtx::reduce_matcher`] — matcher reduction (task 6).
//! - [`MetaCtx::unfold_definition`] — plain delta only this task; task 7
//!   adds smart unfolding + matcher suppression in place.
...
/// oracle: `ReduceMatcherResult` (WHNF.lean:432-436). `Reduced`/`Stuck`/
/// `PartialApp` are only ever constructed by task 6's real
/// `reduce_matcher` body — this task's stub (below) only ever returns
/// `NotMatcher`. Scoped `allow`: a genuine task-6 remnant, not a
/// leftover.
#[allow(dead_code)]
...
impl<'e> MetaCtx<'e> {
    ...
    /// oracle: `reduceMatcher?` (WHNF.lean:536-575). SEAM: task 6
    /// replaces this body with the `reduceMatcher?` transcription
    /// (matcher-info lookup via `MatcherEntry`, alternative selection).
    /// Always `NotMatcher` here.
    pub(crate) fn reduce_matcher(&mut self, _e: ExprId) -> Result<ReduceMatcherResult, MetaError> {
        Ok(ReduceMatcherResult::NotMatcher)
    }
    ...
    /// `Defn`-with-value only, gated by `can_unfold`. No smart
    /// unfolding, no matcher suppression — task 7 wraps this in place.
    ///
