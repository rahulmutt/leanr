...
use crate::bank::names::NameRow;
use crate::bank::terms::Node;
use crate::bank::{ExprId, LevelId, NameId, Store};
use crate::{
...
/// `subst`'s private helper of the same shape).
...
fn get_undef_param_id(
    ...
    match *st.level_row(base, l) {
        crate::bank::levels::LevelRow::Zero | crate::bank::levels::LevelRow::MVar(_) => Ok(None),
        crate::bank::levels::LevelRow::Param(n) => {
            if let Some(id) = n {
                if ps.contains(&id) {
                    return Ok(None);
                }
        ...
        crate::bank::levels::LevelRow::Succ(a) => {
            g.enter(|g| get_undef_param_id(st, base, a, ps, g))
        }
        crate::bank::levels::LevelRow::Max(a, b) | crate::bank::levels::LevelRow::IMax(a, b) => g
            .enter(|g| {
...
impl<'e> TypeChecker<'e> {
    ...
    /// `inductive::run_tc`, which owns the persistent
    ...
    /// the same consumer as `new_with` (`inductive::run_tc`).
    ...
    /// anonymous name" — see `quot_red::QuotCtx::const_name`'s doc
    ...
    /// check as `env`'s free-function port of the same name, but
