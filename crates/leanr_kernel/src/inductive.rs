...
fn level_is_zero(st: &Store, base: Option<&Store>, l: LevelId) -> bool {
    matches!(*st.level_row(base, l), crate::bank::levels::LevelRow::Zero)
}
...
// equivalents of the private methods already in `tc::TypeChecker`
...
impl<'a> AddInductiveFn<'a> {
    ...
    fn get_rec_levels(&self) -> Vec<LevelId> {
        match *self.scratch.level_row(self.base(), self.elim_level) {
            crate::bank::levels::LevelRow::Param(_) => {
    ...
    fn get_rec_lparams(&self) -> Vec<NameId> {
        match *self.scratch.level_row(self.base(), self.elim_level) {
            crate::bank::levels::LevelRow::Param(Some(u)) => {
                let mut ps = vec![u];
