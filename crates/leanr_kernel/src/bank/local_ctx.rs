...
mod tests {
    ...
    use crate::{
        BinderInfo, Expr as ArcExpr, ExprNode as ArcExprNode, FVarIdGen as ArcFVarIdGen,
        LocalContext as ArcLocalContext, Nat,
    };
    use std::sync::Arc;
    ...
    /// ids out with `to_name` and compare against the Arc kernel's own
    /// `FVarIdGen`/`LocalContext` output (oracle: type_checker.cpp:24).
    #[test]
    fn fresh_ids_match_arc_kernel_output() {
        ...
        // Cross-check against the ACTUAL Arc kernel's FVarIdGen/LocalContext.
        let mut arc_gen = ArcFVarIdGen::default();
        let mut arc_lctx = ArcLocalContext::default();
        let arc_ty = ArcExpr::lit(crate::Literal::NatVal(Nat::from(0u64)));
        let arc_fvar0 = arc_lctx.mk_local_decl(
            &mut arc_gen,
            &nm("x"),
            Arc::clone(&arc_ty),
            BinderInfo::Default,
        );
        let arc_fvar1 = arc_lctx.mk_local_decl(&mut arc_gen, &nm("x"), arc_ty, BinderInfo::Default);
        let ArcExprNode::FVar { id: arc_id0 } = arc_fvar0.node() else {
            panic!("expected FVar node")
        };
        let ArcExprNode::FVar { id: arc_id1 } = arc_fvar1.node() else {
            panic!("expected FVar node")
        };
        assert_eq!(name0, *arc_id0);
        assert_eq!(name1, *arc_id1);
    }
