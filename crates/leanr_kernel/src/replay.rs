...
//! is structurally identical (`decl::constant_info_eq`, Task 1) to
...
//! itself has no id equivalent (see `env::Environment::add_core`'s
//! doc comment).
//!
...
impl Replayer<'_> {
    ...
    fn replay_constant_inner(&mut self, name: NameId, g: &mut RecGuard) -> Result<(), KernelError> {
        ...
        // iteration.
        let deps = crate::used_consts::used_constants(self.env.view().store, None, &ci);
        for dep in deps {
