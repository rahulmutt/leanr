...
/// too, leaving only `CoeSort`. Reconciled a SIXTH time by M4b-3 P4
/// task 9: `CoeSort`'s binder-domain arm landed in `ensure_type`
/// (`coe.rs`), so coercion insertion (`CoeT`/`CoeFun`/`CoeSort`,
/// `mkCoe`) is no longer deferred at all:
/// ```text
